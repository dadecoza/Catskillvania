...
typedef struct
{
    bool active;            // True = active (logic can execute, ie enemy moves)
    bool visible;           // True = onscreen false = offscreen. If false, don't run hitbox
    uint8_t state;          // Falling, turning, intact, destroyed, test animate etc
    uint8_t whenBudTouch0;  // 0 = nothing, 1 = knock over, 2 = collect (greenie), 3 = kill (monster)
    uint16_t xPos, yPos;    // Position in world, is upper left corner of sprite
    bool singleTile;        // Greenies are single cell, rest are bigger
    uint8_t sheetX, sheetY; // Upper left corner of sprite on sheet
    uint8_t width;          // Size in tiles
    uint8_t height;         // Used for hitboxes and screen calcs
    uint8_t palette;        // 0-7
    ...
    bool dir;               // False = facing right (not flipped) Sprites should be drawn facing right as default (false)
    bool turning;
    uint8_t animate;
    uint8_t subAnimate;
    uint16_t xSentryLeft;
    uint16_t xSentryRight;
    bool moving;
    bool extraY;
    uint8_t extraA;
    uint8_t extraB;
...
enum stateMachineEdit editState = tileDrop;
...
enum mapNames currentMap = null;
enum mapNames lastMap = null; // Store currentMap here when Bud Dies, since Jail is also a location. After jail, goto lastMap
...
// Master loops
void setup()
{ //------------------------Core0 handles the file system and game logic
...
void setup1()
{ //-----------------------Core 1 builds the tile display and sends it to the LCD
}
...
void loop1()
{ //------------------------Core 1 handles the graphics blitter. Core 0 can process non-graphics game logic while a screen is being drawn
...
void gameLoopLogic()
{
    ...
    case 2: // Done with our audio, now waiting for frame to finish rendering (we can start our vid mem logic during the final DMA)
        if (getRenderStatus() == false)
        {                      // Wait for core 1 to finish drawing frame
...
void gameFrame()
{
    ...
    case bootingMenu: // Special type of load that handles files not being present yet
        if (menuTimer == 65534)
        {                       // Let one frame be drawn to show the LOAD USB FILES message then...
            displayPause(true); // Spend most of the time NOT drawing the screen so files can load easier
        }

        if (menuTimer > 0)
        { // If timer active, decrement
            --menuTimer;
        }

        if (menuTimer == 0)
        { // First boot, or did timer reach 0?
            if (loadRGB("UI/NEStari.pal"))
            { // Try and load master pallete from root dir (only need to load this once)
            ...
            {                        // No files? Make USB file load prompt using flash text patterns and wait for user reset
    ...
    case splashScreen:
        ...
        if (--menuTimer == 0)
        { // Dec timer and goto main menu after timeout
    ...
    case titleScreen:
        ...
        if (menuTimer == 20)
        { // Bud blinks
            drawTileXY(2, 11, 8, 14, 4, 0);
            drawTileXY(3, 11, 9, 14, 4, 0);
            drawTileXY(2, 12, 8, 15, 4, 0);
            drawTileXY(3, 12, 9, 15, 4, 0);

            drawTileXY(5, 10, 10, 14, 4, 0);
            drawTileXY(6, 10, 11, 14, 4, 0);
            drawTileXY(5, 11, 10, 15, 4, 0);
            drawTileXY(6, 11, 11, 15, 4, 0);
        }

        if (menuTimer == 0)
        { // Un-blink face in the lazier way possible
            menuTimer = rnd(50, 150);

            for (int y = 8; y < 16; y++)
            { // Draw BUD FACE in palette 4
                for (int x = 0; x < 8; x++)
                {
                    drawTileXY(x, y - 1, x, y, 4, 0);
                }
            }
        }
    ...
    case levelEdit:
        ...
        if (editEntryFlag == true)
        { // user must release A button before we can edit (to avoid false drops after removing debounce)
    ...
    if (displayPauseState == false)
    { // Screen NOT paused?
        if (button(start_but))
        { // Button pressed?
            if (gameState == goCondo || gameState == goHallway)
...
void diffSelectLogic()
{
    ...
    case 1:
        drawSpriteRange(28, 28 + budY, 0, 16, 8, 8, 4, false, false);
        drawText("     KITTEN", 0, 13, false);
        break;

    case 2:
        drawSpriteRange(28, 28 + budY, 8, 16, 8, 8, 4, false, false);
        drawText("      CAT", 0, 13, false);
        break;
        // 0123456789ABCDE				//Draw score centered
    case 3:
        drawSpriteRange(28, 28 + budY, 0, 16 + 8, 8, 8, 4, false, false);
...
void jailLogic()
{
    ...
    drawSpriteRange(40, jailYpos, 8, 16 + 10, 5, 5, 7, false, false);

    drawSpriteTile(40, 80, 0x01F8, 0, false, false);
    drawSpriteTile(56, 80, 0x01F9, 0, false, false);
    ...
    if (menuTimer > 50)
    {
        drawSpriteRange(52, 47, 13, 16 + 13, 2, 3, 4, false, false); // Sitting
    }
    else
    {
        drawSpriteRange(52, 47, 13, 16 + 10, 2, 3, 4, false, false); // Paws on bars
    }
...
void setupCondo(int whichFloor, int whichCondo)
{
    ...
    for (int x = 0; x < maxThings; x++)
    { // Find first open slot
        if (object[x].active == true)
        { // && object[x].category == 0) {		//If object exists and is a robot, turn movement on/off
    ...
    for (int x = 0; x < maxThings; x++)
    { // Find first open slot
        if (object[x].active == true && object[x].category == 0 && object[x].type == 4)
        { // Count kittens
            kittenToRescue++;
        }
    }
...
void redrawMapTiles()
{ // Redraws all strips into tile map memory
    ...
    {                                          // Draw 26 columns
        ...
        ++tileColumn;
        tileColumn &= 0x1F; // Inc with binary rollover (32 wide) JLR??

        if (++mapColumn == mapWidth)
        { // Rollover (static as it's always a single inc no remainder)
            mapColumn = 0;
        }
    }
...
void condoLogic()
{
    ...
    if (budState != dead)
    { // Messages top sprite pri, but don't draw if Bud dying
        if (kittenMessageTimer > 0)
        {
            ...
            if (kittenMessageTimer & 0x04)
            {
                ...
                else
                {
                    ...
                    if (kittenCount == (kittenToRescue - 1))
                    { // Only one KITTENS left? Draw a sloppy slash over the S
        ...
        if (kittenCount == kittenToRescue)
        {
            ...
            drawSpriteRange(7 - inArrowFrame, 88, 6, 16 + 12, 1, 2, 4, false, false); // out arrow sprite <
            drawSpriteRange(16, 84, 7, 16 + 11, 1, 3, 4, false, false);               // OUT text
...
void setupHallway()
{
    ...
    placeObjectInMapNumber(32, 48, 4, 32 + 12, 4, 3, 6, true, 10, 5, 21); // Spawn an intact window on object #21

    if (currentFloor == 1)
    { // Broken window only on first floor
        if (currentCondo > 0)
        { // Bud has already been in a condo? (not game opening)
            if (currentFloor == 1)
            {
    ...
    placeObjectInMapNumber(1025, 48, 4, 32 + 12, 4, 3, 6, true, 10, 5, 23); // Other window spawns as object #23
...
{ //--------------------This is called at 30Hz. Your main game state machine should reside here
    ...
    if (budState == entering)
    { // If Bud is entering elevator or condo, draw robots first so they appear in front of him
        objectLogic();
    }
    ...
    else
    {
        if (++budSpawnTimer == 20)
        {
            ...
            placeObjectInMapNumber(32, 40, 0, 32 + 8, 4, 3, 6, true, 10, 6, 22); // Spawn broken glass as object #22
    ...
    if (robotsToSpawn > 0)
    {
        ...
        if (--robotsSpaceTimer == 0)
        {
            ...
            robotsSpaceTimer = 90 + rnd(0, 50); // Reset timer
            robotsToSpawn--;                    // Dec #
            ...
            if (budWorldX < 512)
            { // If Bud on left side of map, spawn on right
                x = 700;
                dir = true;
            }
            ...
            // int robot = placeObjectInMap(x, 64, 0, 48 + 0, 2, 6, rnd(4, 8), dir, 0, 0);
    ...
    if (budState != entering)
    { // Default draw Bud first so he appears over things like chandeliers
        objectLogic();
    }
    else
    {
        if (waitPriorityChange == true)
        {                               // Bud's state just changed, and we need one more frame before flipping priority?
...
void spawnHallwayRobot(int spawnXpos, bool direction)
{
    ...
    selectRobot = rnd(0, 4); // Get random robot #

    int x;

    for (x = 0; x < maxThings; x++)
    { // Find first open slot
    ...
    object[objectIndexNext].palette = rnd(4, 8);
    ...
    if (selectRobot == 2)
    { // Flat robots don't fix an 8 pixel edge, drop by 4
...
void objectLogic()
{
    ...
    for (int g = 0; g < highestObjectIndex; g++)
    { // Scan no higher than # of objects loaded into level SCAN ALL? BJH

        if (object[g].active)
        {
            go_scan(g, worldX + xShake, 0 + yShake); // Draw object if active, plus logic (in object) Robots move if off-camera

            if (object[g].state == 99 && object[g].moving == true)
            { // Object falling?
                object[g].yPos += object[g].animate;

                if (object[g].animate < 8)
                {                        // Object has to reach full fall speed in order to break or hit a robot
                    object[g].animate++; // Keep approaching TERMINAL VELOCITY
                }

                if (objectToTile(g) == true && object[g].animate > 6)
                { // Scan base of object and see if it hit something solid (as wide as itself)
                    ...
                    object[g].sheetX = 8 + rnd(0, 5);                // Turn object to rubble
            ...
            if (object[g].state == 100)
            { // Object hit floor?
                if (object[g].type == 50)
                { // Chandelier? Big sound
                    playAudio("audio/glass_2.wav", 100);
                }
                else
                {
            ...
            if (object[g].visible)
            {
                ...
                if (object[g].category == 0)
                {
                    ...
                    { // Robot?
                        ...
                        default:
                            if (go_hitBox(g, budWx1, budWy1, budWx2, budWy2) == true && budBlink == 0 && budStunned == 0)
                            { // Did it hit Bud?
                                budDamage();
                            }
                    ...
                    if (object[g].type == 4 && object[g].state == 0)
                    { // Un-rescued kitten?

                        if (go_hitBoxSmall(g, budWx1, budWy1, budWx2, budWy2) == true)
                        {
                            score += 50;
                    ...
                    if (object[g].type == 5 && budPower < powerMax)
                    { // Greenie? Only run logic if Bud needs power (no pickup on full power)
                ...
                { // Bud is swiping?
                    ...
                    if (object[g].category > 0 && object[g].state == 0)
                    { // Bud swipes inert objects

                        if (go_hitBox(g, budAx1, budAy1, budAx2, budAy2) == true)
                        {
                    ...
                    { // Gameplay object?
                        ...
                        if (object[g].type == 4 && object[g].state == 0)
                        { // If bud hits a kitten...

                            if (object[g].xSentryLeft == 0 && go_hitBox(g, budAx1, budAy1, budAx2, budAy2) == true)
                            {
                        ...
                        if (object[g].type < 4 && object[g].state == 200)
                        { // If a robot is in stun state

                            if (go_hitBox(g, budAx1, budAy1, budAx2, budAy2) == true)
                            { // Punch landed? You're terminated!
                ...
                if (object[g].category == 1 && jump > 0 && object[g].state == 0)
                { // Bud can knock loose bad art and chandeliers by jumping through it

                    if (go_hitBox(g, budWx1, budWy1, budWx2, budWy2) == true)
                    {
...
void storyLogic()
{
    ...
    case 0:
        drawSpriteRange(16, 16, 4, 16, 3, 4, 4, false, false); // Report eyes and hair glint

        cutAnimate++;

        if (cutAnimate == 8)
        {
            drawTileXY(3, 5, 0, 0, 0, 0);
            drawTileXY(4, 5, 1, 0, 0, 0);
            drawTileXY(3, 6, 0, 1, 0, 0);
            drawTileXY(4, 6, 1, 1, 0, 0);
        }
        if (cutAnimate == 16)
        {
            drawTileXY(3, 5, 2, 0, 0, 0);
            drawTileXY(4, 5, 3, 0, 0, 0);
            drawTileXY(3, 6, 2, 1, 0, 0);
            drawTileXY(4, 6, 3, 1, 0, 0);
            cutAnimate = 0;
        }
        menuTimer--;
    ...
    case 2:
        ...
        if (cutAnimate == 8)
        { // Scared face
            drawTileXY(3, 5, 4, 0, 0, 0);
            drawTileXY(4, 5, 5, 0, 0, 0);
            drawTileXY(3, 6, 4, 1, 0, 0);
            drawTileXY(4, 6, 5, 1, 0, 0);
        }
        if (cutAnimate == 16)
        {
            drawTileXY(3, 5, 6, 0, 0, 0);
            drawTileXY(4, 5, 7, 0, 0, 0);
            drawTileXY(3, 6, 6, 1, 0, 0);
            drawTileXY(4, 6, 7, 1, 0, 0);
            cutAnimate = 0;
        }
        ...
        if (cutSubAnimate2 > 0)
        {
            ...
            drawSpriteRange(budX, budY, cutSubAnimate2, 16, 2, 2, 3, false, false);

            if (menuTimer & 0x01)
            { // Advance explosion graphic on 2's
                cutSubAnimate2 += 2;
                if (cutSubAnimate2 == 14)
                { // 3 frames done? End (until respawn)
                    cutSubAnimate2 = 0;
                }
            }
        }
    ...
    case 5:
        ...
        if (menuTimer == 35)
        {
            for (int y = 0; y < 3; y++)
            {
                ...
                for (int x = 0; x < 4; x++)
                {
                    drawTileXY(x + 5, y + 3, x + 12, y + 9, 0, 0);
                }
            }
        }
...
void endingLogic()
{
    ...
    for (int g = 0; g < 9; g++)
    { // Scroll slices, leave bottom text static
    ...
    drawSpriteRange(12, 59, 3, 16 + (budFrame << 1), 3, 2, 4, true, false); // Bud walking

    drawSpriteRange(40, 59, 12, 16 + (kittenAnimate << 1), 2, 2, 5, true, false); // Kittens

    drawSpriteRange(57, 59, 12, 16 + (kittenAnimate << 1), 2, 2, 7, true, false); // Kittens

    drawSpriteRange(77, 59, 12, 16 + (kittenAnimate << 1), 2, 2, 5, true, false); // Kittens

    drawSpriteRange(100, 59, 12, 16 + (kittenAnimate << 1), 2, 2, 4, true, false); // Kittens
...
{ // THANKS CHAT-GPT! OUR EVIL SKYNET OVERLORD
...
void drawBudStats()
{
    ...
    drawSpriteTile(4, 4, 0x01F8, 4, false, false);  // face
    drawSpriteTile(12, 4, 0x01F9, 0, false, false); // X
    ...
    for (int x = 0; x < powerMax; x++)
    {
        if (budPower > x)
        {
            drawSpriteTile((x << 3) + 4, 14, 0x01FB, 4, false, false);
        }
        else
        {
            drawSpriteTile((x << 3) + 4, 14, 0x01FA, 4, false, false);
        }
    }
}
...
{ // Pass in object to fall

    for (int x = 0; x < 16; x++)
    {

        if (fallingObjectState[x] != true)
        { // Find firt empty slot
...
{ // Pass in falling object to remove
...
{ // Pass in robot object index we are checking for falls against
    ...
    for (int x = 0; x < 16; x++)
    {
        ...
        { // Object is falling?
            ...
            if (object[g].animate > 5)
            { // Object must be falling at at least half speed to kill a robot
                ...
                // }
                // else {
                if (go_hitBoxSmall(g, object[g].xPos, object[g].yPos, object[g].xPos + (object[g].width << 3), object[g].yPos + (object[g].height << 3)) == true)
                {
                    hit = true;
                }
                if (go_hitBox(g, object[g].xPos, object[g].yPos, object[g].xPos + (object[g].width << 3), object[g].yPos + (object[g].height << 3)) == true)
                {
                    hit = true;
                }
...
void movingObjects(bool state)
{
    ...
    { // Find first open slot
        if (object[x].active == true)
        { // If object exists and is a robot, turn movement on/off
...
{ // Returns the tile flags nearest the given global world x y position. Allows objects to land on platforms other than the floor
    ...
    for (int i = 0; i < object[g].width; i++)
    { // Object must hit an edge along its entire width to land (so edge of counter it keeps going)
        if ((condoMap[y + (object[g].height - 1)][x + i] & 0xF800) != 0)
        {                // Plat or block bits?
            edgeCount++; // Inc count
        }
    }
...
void budLogic2()
{
    ...
    if (budState == dead)
    {
        ...
        if (budStunnedDir == true)
        {                                                                                              // Stunned rolling left
            drawSpriteRange(budX, budY - 8, 14, 16 + (budFrame << 1), 2, 2, budPalette, false, false); // Rolls left, but is facing right, rolling backwards
        }
        else
        { // Stunned rolling right
            drawSpriteRange(budX, budY - 8, 14, 16 + (budFrame << 1), 2, 2, budPalette, true, false);
        }
    ...
    if (budBlink > 0)
    {
        ...
        {
            if (budBlink & 0x01)
            { // If blink active, use alt Bud palette on ones (invinc flash)
                budPalette = 8;
            }
        }
        else
        {
            if (budBlink & 0x02)
            { // Blink slower when time almost up
                budPalette = 8;
            }
        }
    ...
    case rest: // Rest = not moving left or right. Can still be jumping or falling
        if (jump > 0)
        {
            ...
            { // Falling facing right
                drawSpriteRange(budX, budY - 8, 0, 16 + offset, 3, 2, budPalette, budDir, false);
                setBudHitBox(budWorldX, budWorldY - 8, budWorldX + 24, budWorldY + 8);
            }
            else
            {
                drawSpriteRange(budX - 8, budY - 8, 0, 16 + offset, 3, 2, budPalette, budDir, false); // Falling facing left
        ...
        else
        {
            ...
            if (budDir == false)
            {                                                                          // Right
                drawSpriteSingle(budX, budY, 0 + tail, 31, budPalette, budDir, false); // Draw animated tail on fours
                drawSpriteSingle(budX + 8, budY, 7, 18, budPalette, budDir, false);    // Front feet
                drawSpriteSingle(budX, budY - 8, 6, 17, budPalette, budDir, false);    // Back

                if (mewTimer > 0)
                {
                    mewTimer--;
                    drawSpriteSingle(budX + 8, budY - 16, 7, 16 + 8, budPalette, budDir, false); // Ear tips
                    drawSpriteSingle(budX + 8, budY - 8, 7, 16 + 9, budPalette, budDir, false);  // Mew mouth
                }
                else
                {
                    if (blink < 35)
                    {
                        drawSpriteSingle(budX + 8, budY - 8, 7, 17, budPalette, budDir, false);
                    }
                    else
                    {
                        drawSpriteSingle(budX + 8, budY - 8, 6, 16, budPalette, budDir, false); // Blinking Bud!
                    }

                    drawSpriteSingle(budX + 8, budY - 16, 7, 16, budPalette, budDir, false); // Ear tips
                }
            }
            else
            {                                                                              // Left
                drawSpriteSingle(budX + 8, budY, 0 + tail, 31, budPalette, budDir, false); // Draw animated tail on fours
                drawSpriteSingle(budX, budY, 7, 18, budPalette, budDir, false);            // Front feet
                drawSpriteSingle(budX + 8, budY - 8, 6, 17, budPalette, budDir, false);    // Back

                if (mewTimer > 0)
                {
                    mewTimer--;
                    drawSpriteSingle(budX, budY - 16, 7, 16 + 8, budPalette, budDir, false); // Ear tips
                    drawSpriteSingle(budX, budY - 8, 7, 16 + 9, budPalette, budDir, false);  // Mew mouth
                }
                else
                {
                    if (blink < 35)
                    {
                        drawSpriteSingle(budX, budY - 8, 7, 17, budPalette, budDir, false);
                    }
                    else
                    {
                        drawSpriteSingle(budX, budY - 8, 6, 16, budPalette, budDir, false); // Blinking Bud!
                    }

                    drawSpriteSingle(budX, budY - 16, 7, 16, budPalette, budDir, false); // Ear tips
                }
            }
            if (animateBud)
            {
    ...
    case moving:
        if (jump > 0)
        {                   // If jumping while moving use the jump stills
            int offset = 6; // Jumping up gfx (default)
            if (jump == 128)
            {
                offset = 8; // Falling down gfx
        ...
        { // Running on ground animation

            if (budDir == false)
            {
                drawSpriteRange(budX, budY - 8, 0, 16 + (budFrame << 1), 3, 2, budPalette, budDir, false); // Running
                setBudHitBox(budWorldX, budWorldY - 8, budWorldX + 24, budWorldY + 6);
                budMoveRightC(3);
            }
            else
            {
                drawSpriteRange(budX - 8, budY - 8, 0, 16 + (budFrame << 1), 3, 2, budPalette, budDir, false); // Running
                setBudHitBox(budWorldX - 8, budWorldY - 8, budWorldX + 16, budWorldY + 6);
                budMoveLeftC(3);
            }
            if (animateBud)
    ...
    case swiping:
        if (budDir == true)
        { // Left
            drawSpriteRange(budX - 16, budY - 16, 8, 16 + (budSwipe * 3), 4, 3, budPalette, budDir, false);
            setBudHitBox(budWorldX, budWorldY - 8, budWorldX + 16, budWorldY + 6);
            setBudAttackBox(budWorldX - 16, budWorldY - 12, budWorldX, budWorldY);
        }
        else
        { // Right
            drawSpriteRange(budX, budY - 16, 8, 16 + (budSwipe * 3), 4, 3, budPalette, budDir, false);
            setBudHitBox(budWorldX, budWorldY - 8, budWorldX + 16, budWorldY + 6);
    ...
    case exiting: // Coming OUT of a condo or elevator and going INTO hallway
        ...
        if (budVisible == true)
        {
            drawSpriteRange(budX + 4, budY - 8, 7, 16 + 5, 1, 2, 4, budDir, false);
        }
        if (animateBud)
        {
            if (budFrame & 0x02)
    ...
    case entering: // Going into a condo or elevator
        moveSuppress = true;
        drawSpriteRange(budX + 4, budY - 8, 7, 16 + 3, 1, 2, 4, budDir, false);
        if (animateBud)
        {
            if (budFrame & 0x02)
            {
    ...
    { // If Bud isn't frozen (enter/exit/dead) then player can move him

        if (jump & 0x7F)
        { // Jump is set, but the MSB (falling) isn't? Must be rising still
            if (++jump < 12)
            { // Jump up with decreasing velocity
                budY -= velocikitten;
            ...
            { // Hit top? Set falling and reset velocity
                jump = 128;
                velocikitten = 2;
            }
        }
        ...
        if (jump == 0 || jump == 128)
        { // Walked off a ledge or coming down from a jump?

            bool fallEdge = false;

            if (budY > -1)
            { // Only do checks with Bud on screen (else tile data is bad)
                if (getTileType(budTileCheck(0), (budY >> 3)) == 0 && getTileType(budTileCheck(1), (budY >> 3)) == 0)
                { // Nothing below?
                    fallEdge = true;
                }
            }
            ...
            {
                if (jump == 128)
                {             // Were we falling? Clear flag
                    jump = 0; // Something below? Clear jump flag
                    budY &= 0xF8;
                    budWorldY &= 0xF8;

                    if (budStunned == 0)
                    { // If falling while stunned, don't reset state (stun timeout will)
                        budState = rest;
                    }
                }
            }
        ...
        if (button(B_but) && jump == 0)
        { // Can't swipe while jumping
    ...
    if (budNoMove == true && budState == moving)
    { // Was Bud moving, but then d-pad released? Bud comes to a stop
        budState = rest;
        budFrame = 0;
    }

    if (currentMap == hallway)
    { // In the hallway? See if Bud can enter a door or elevator

        int temp = checkDoors(); // See if Bud is standing in front of a door

        if (temp > -1 && budState != entering)
        { // He is? tempCheck contains a index # of which one (0-5)

            if ((apartmentState[temp] & 0xC0) == 0x00)
            { // If apartment cleared or door already open bits set, no action can be taken
                ...
                drawSpriteRange(budX, budY - 28 - (inArrowFrame >> 1), 6, 16 + 14, 2, 1, 4, false, false); // IN ARROW sprites
                drawSpriteRange(budX, budY - 20, 6, 16 + 15, 2, 1, 4, false, false);                       // IN ARROW sprites
        ...
        if (checkElevator() == 1 && budState != entering && budState != exiting)
        { // Bud standing in front of elevator?
...
bool budMoveLeftC(int speed)
{
    ...
    if (budY > -1)
    { // Tile obstacle check first
        if ((getTileType(budTileCheck(-1), (budY >> 3)) & tileBlocked) == tileBlocked)
        { // Check 1 to the left
            return false;
        }
    }

    if (condoMoveLeft(speed) == false)
    { // Can window NOT scroll left?

        if (budY > 56)
        { // Bud can move further left than 8px if he's inside the door (to exit or be taunted)
            budX -= speed;
            budWorldX -= speed;
            xWindowBudFine -= speed;

            if (xWindowBudFine < 0)
            {                        // Passed tile boundary?
...
bool budMoveRightC(int speed)
{
    ...
    if (budY > -1)
    { // Tile obstacle check first
        if ((getTileType(budTileCheck(2), (budY >> 3)) & tileBlocked) == tileBlocked)
        { // Check 2 to the right
            return false;
        }
    }

    if (condoMoveRight(speed) == false)
    { // Can window NOT scroll right?

        if (budX < 98)
        { // Prevent from jumping over right edge of the world (DAMN SPEED RUNNERS!)
            budX += speed;
            budWorldX += speed;
            xWindowBudFine += speed;
...
int budTileCheck(int whereTo)
{
    ...
    if (whereTo < 0)
    { // Negative? (checking left)
        temp += whereTo;

        if (temp < 0)
        { // Acount for rollovers
            temp += 32;
        }
    }
    else
    { // Positive (checking right)
        temp += whereTo;
...
{ // Low side -2 pixels so robots passing under don't kill Bud (his ears at rest also null)
...
int checkDoors()
{
    ...
    if (budY != 104)
    { // Doors only ping if Bud is on floor
        return -1;
    }
    ...
    for (int x = 0; x < 6; x++)
    {
        ...
        if (tileX >= doorTilePos[x] && tileX <= (doorTilePos[x] + 1))
        { // Is Bud aligned with a door?
            return x;
        }
    }
...
int checkElevator()
{
    ...
    if (budY != 104)
    { // Elevator only pings if Bud is on floor
        return -1;
    }

    if (tileX >= 65 && tileX <= 68)
    { // Is Bud aligned with elevator?
        return 1;
    }
...
{ // whichDoorNum is 1-6
    ...
    for (int col = 0; col < 4; col++)
    {
        ...
        if (col == 1)
        {
            if (apartmentState[whichDoorNum - 1] == 0)
            {                                                          // Apartment not clear? (MSB set)
...
{ // Counter + window (black BG covered with sprite)
...
{ // Same as the one in library but it draws to condomap memory
...
void populateEditCondo()
{
    ...
    for (int x = 0; x < mapWidth; x++)
    { // Platform (blank, one above floor)
        condoMap[13][x] = ' ' | (3 << 8);
    }
    for (int y = 0; y < 13; y++)
    {
...
void populateEditDoors()
{
    ...
    for (int x = 0; x < 6; x++)
    {
        ...
        if (apartmentState[x] & 0x40)
        { // Door open bit set? (bud emerging from condo)
...
void drawEditMenuPopUp()
{
    ...
    drawDecimalXY(currentCopyBuffer, 14, 0);
    drawText("PASTE + SLOT", 1, 1, false);
    drawDecimalXY(currentCopyBuffer, 14, 1);
    // 0123456789ABCDE
    drawText("---------------", 1, 2, false);
    ...
    if (editType == false)
    { // If editing a hallway, the condo # must be 0
        condoZero = 48;
...
{ // Call thise in edit/game mode before calling save/load
...
{ // Call thise in edit/game mode before calling save/load
    ...
    if (editType == true)
    { // Condo? Change the numbers on the open door on the left
...
void saveLevel()
{
    ...
    for (int y = 0; y < 15; y++)
    { // Write the map file in horizontal strips from left to right, top to bottom
    ...
    for (int x = 0; x < maxThings; x++)
    { // Count objects (they may not be contiguous)
        if (object[x].active == true)
        {
            objectCount++;
        }
    ...
    for (int x = 0; x < maxThings; x++)
    { // Greenie scan (top draw priority)
        if (object[x].active == true)
        {

            if (object[x].category == 0)
            { // Gameplay object?
                if (object[x].type == 5)
                { // Greenie? Save first
                    saveObject(x);
    ...
    { // Tall Robot scan (next priority)
        if (object[x].active == true)
        {
            ...
            { // Gameplay object?
                if (object[x].type == 0)
                { // Tall robot? Save these first (so robots on counters/tables etc appear behind them)
                    saveObject(x);
                }
            }
        }
    ...
    for (int x = 0; x < maxThings; x++)
    { // Robot scan (next priority)
        if (object[x].active == true)
        {

            if (object[x].category == 0)
            { // Gameplay object?
                if (object[x].type > 0 && object[x].type < 4)
                { // All robots other than the tall ones? Save next
                    saveObject(x);
                }
            }
        }
    ...
    for (int x = 0; x < maxThings; x++)
    { // Most other objects (will draw behind kittens, robots and greenies)
        if (object[x].active == true)
        {

            if (object[x].category > 1)
            { // Not a gameplay object or bad art?
                saveObject(x);
            }
        }
    }
    ...
    for (int x = 0; x < maxThings; x++)
    { // Bad art (lowest priority, on walls)
        if (object[x].active == true)
        {
...
void loadLevel()
{
    ...
    for (int y = 0; y < 15; y++)
    { // Read the map file in horizontal strips from left to right, top to bottom
    ...
    if (readByte() == 128)
    { // 128 = end of tile data. Old files don't have this so skip objects if not found
        ...
        {                             // Load that many objects into object RAM. They'll populate from 0 up
    ...
    if (readByte() == 255)
    { // Check end (this should be a checksum)
        closeFile();
        redrawEditWindow();
        makeMessage("LOADED");
...
void editLogic()
{
    ...
    if (button(B_but))
    { // Selecting tiles or sprites?

        if (bButtonFlag == false)
        {                                       // Just switched to menus? Change debounces
            setButtonDebounce(up_but, true, 1); // Debounce unless scrolling map left or right
            setButtonDebounce(down_but, true, 1);
            setButtonDebounce(left_but, true, 1);
            setButtonDebounce(right_but, true, 1);
    ...
    { // Dropping things
        ...
        {                                        // We were in the B menu?
            ...
            if (cursorDrops == 2)
            { // Dropping objects, enable debounce so they don't pile up
                editDisableCursor = false;
                editAaction = 10;
        ...
        if (cursorMenuShow == true)
        { // B released and now we're back here? Redraw window
        ...
        {
            cursorMoveTimer++;
            if (cursorMoveTimer == 1 || cursorMoveTimer > 15)
            { // Did it just start, or was held a while? Allow movement

                if (drawFine == true)
                {
                    if (drawXfine > 0)
                    { // Window doesn't scroll in fine mode
                        drawXfine -= 1;
                    }
                }
                else
        ...
        if (noMove == true)
        { // No movement on dpad? Reset timer
            cursorMoveTimer = 0;
        }

        if (button(C_but))
        { // Cycle through drop types
            if (++cursorDrops > 2)
            {
                cursorDrops = 0;
            }
    ...
    if (manualAdebounce == true)
    { // Flag that A must be released before it can be pressed again?
        if (button(A_but) == false)
        {                            // Is button open?
    ...
    if (button(A_but) && manualAdebounce == false && windowActive == false)
    {
        ...
        case 10:
            ...
            {
            case 0:                     // Game stuff
                placeObjectInMapEdit(); // TEMP BJH - it mostly works?

                if (selectRobot < 4)
                { // Evil robot? Prompt to set sentry edges
                    editAaction = 11;
        ...
        case 11:                                            // Set sentry left edge
            ...
            if (sentryLeft >= object[objectIndexLast].xPos)
            { // Sentry left has to be at least 1 tile to the left of where you dropped robot
        ...
        case 12:                                                 // Set sentry right edge
            ...
            if (sentryRight < object[objectIndexLast].xPos + ((object[objectIndexLast].width + 1) << 3))
            { // Sentry area must be AT LEAST width of robot + 1 tile
    ...
    { // Can cursor be shown?
        ...
        if (windowActive == true)
        {
            if (cursorDrops == 0)
            {                                                                                         // Dropping tiles?
                drawSpriteTile(drawX << 3, drawY << 3, nextTileDrop, tilePlacePalette, false, false); // Just draw next tile, no reticle as we can't move anyway. Makes it easier to see how it fits
            }
            if (cursorDrops == 1)
            {
                switch (tileDropAttribute)
                {
                case 3:
                    drawSpriteTile(drawX << 3, drawY << 3, '.', 0, false, false); // plat
                    break;

                case 4:
                    drawSpriteTile(drawX << 3, drawY << 3, ',', 0, false, false); // blocked
                    break;

                default:
                    drawSpriteTile(drawX << 3, drawY << 3, 64, 7, false, false); // Reticle (free, null)
                    break;
                }
            }
        }
        else
        {
            if (cursorBlink & 0x01)
            {
                if (drawFine == false)
                {
                    drawSpriteTile(drawX << 3, drawY << 3, 64, 7, false, false); // Alternate between reticle and object to drop
                }
            }
            else
            {
                ...
                {
                case 0:
                    drawSpriteTile(drawX << 3, drawY << 3, nextTileDrop, tilePlacePalette, false, false); // Next tile (as sprite)
                    break;
                ...
                case 2:
                    ...
                    {
                        drawSpriteTile(drawX << 3, drawY << 3, 0x2C, 3, false, false);
                    }
                    else
                    {
                        if (drawFine == true)
                        {
                            drawSpriteRange(drawXfine, drawYfine, objectSheetIndex[objectDropCategory][0], objectSheetIndex[objectDropCategory][1], objectTypeSize[objectDropCategory][0], objectTypeSize[objectDropCategory][1], objectPlacePalette, objectPlaceDir, false);
                        }
                        else
                        {
                            drawSpriteRange(drawX << 3, drawY << 3, objectSheetIndex[objectDropCategory][0], objectSheetIndex[objectDropCategory][1], objectTypeSize[objectDropCategory][0], objectTypeSize[objectDropCategory][1], objectPlacePalette, objectPlaceDir, false);
                        }
                    }
    ...
    { // Don't draw sprite objects in menu
        editorDrawObjects();
    }
}
...
{ // Used to spawn an item at a forced object # (used for windows in hallway to ensure they appear BEHIND robots)
...
void testAnimateObjects(bool onOff)
{
    ...
    for (int x = 0; x < maxThings; x++)
    { // Scan all possible
        if (object[x].active == true && object[x].category == 0)
        { // If object exists and is a robot, turn movement on/off
...
void editorDrawObjects()
{
    ...
    for (int g = 0; g < maxThings; g++)
    {
        ...
        if (object[g].active)
        {
            go_scan(g, editWindowX << 3, 0); // See if object visible

            if (objectDropCategory == 5 && eraseFlag == true)
            {

                if (go_hitBox(g, (editWindowX << 3) + (drawX << 3), drawY << 3, (editWindowX << 3) + (drawX << 3) + 8, (drawY << 3) + 8) == true)
                { // Does the 8x8 reticle fall within the object bounds?
                    object[g].active = 0;
                    eraseFlag = false;
...
void windowTileSelect()
{
    ...
    if (cursorBlink & 0x01)
    {
        drawSpriteTile((atX + 3) << 3, (atY + 3) << 3, 64, 7, false, false); // Draw reticle on center select tile
    }
...
void drawObjectLoadBrush(bool inMenu)
{
    ...
    case 0:
        ...
        if (inMenu)
        {
            ...
            drawSpriteRange(0, 0, objectSheetIndex[objectDropCategory][0], objectSheetIndex[objectDropCategory][1], objectTypeSize[objectDropCategory][0], objectTypeSize[objectDropCategory][1], objectPlacePalette, objectPlaceDir, false);
        }
        break;
    case 1:
    ...
    case 5:
        if (inMenu)
        {
            ...
            drawSpriteTile(16, 16, 0x2C, 3, false, false);
        }
        break;
    case 6:
        if (inMenu)
...
{
    object[index].xPos = atX;
    object[index].yPos = atY;
}
void go_defineTiles(int index, int x1, int y1, int x2, int y2)
{
}
void go_defineTileSingle(int index, int whichTile)
{
}
...
void go_scan(int index, uint16_t worldX, uint16_t worldY)
{
    ...
    if (object[index].xPos + (object[index].width << 3) < worldX)
    { // Would object appear in window (left edge X + width of object)
        object[index].visible = false;
    }

    if (object[index].xPos > (worldX + 119))
    { // Is object past right edge of window?
        object[index].visible = false;
    }
    ...
    int tempX = object[index].xPos;

    // 0= tall robot 1=can robot, 2 = flat robot, 3= dome robot, 4= kitten, 5= greenie (greenies on bud sheet 1)
    if (object[index].category == 0)
    { // Gameplay objects?

        if (object[index].type < 4 && object[index].moving == true)
        { // Evil robuts
            ...
            {                                  // State 200 = stunned
                ...
                if (object[index].stunTimer > 20)
                { // Shakes more early on, when shaking slows down stun time almost up!
                    shake = 2;
                }

                if (object[index].stunTimer > 50)
                { // Shakes more early on, when shaking slows down stun time almost up!
                    shake = 3;
                }

                if (object[index].animate & 0x01)
                { // Swap faces and shake left and right
                    ...
                    {
                    case 0:
                        drawSpriteRange(tempX - worldX, object[index].yPos - worldY, object[index].sheetX + 4, object[index].sheetY, object[index].width, 3, object[index].palette, object[index].dir, false); // Just the blank face
                        break;

                    case 1:
                        drawSpriteRange(tempX - worldX, object[index].yPos - worldY, object[index].sheetX + 0, object[index].sheetY + 3, object[index].width, 1, object[index].palette, object[index].dir, false); // Just the eyes (top row)
                        break;

                    case 2:
                        drawSpriteRange(tempX - worldX, object[index].yPos - worldY, object[index].sheetX + 0, object[index].sheetY + 2, object[index].width, 1, object[index].palette, object[index].dir, false); // Just the eyes (top row)
                        break;

                    case 3:
                        drawSpriteRange(tempX - worldX, object[index].yPos - worldY, object[index].sheetX + 0, object[index].sheetY + 3, object[index].width, 2, object[index].palette, object[index].dir, false); // Just the eyes (top row)
                        break;
                    }
                }
                else
                {
                    ...
                    case 1:
                        drawSpriteRange(tempX - worldX, object[index].yPos - worldY, object[index].sheetX + 3, object[index].sheetY + 3, object[index].width, 1, object[index].palette, object[index].dir, false); // Just the eyes (top row)
                        break;

                    case 2:
                        drawSpriteRange(tempX - worldX, object[index].yPos - worldY, object[index].sheetX + 4, object[index].sheetY + 2, object[index].width, 1, object[index].palette, object[index].dir, false); // Just the eyes (top row)
                        break;

                    case 3:
                        drawSpriteRange(tempX - worldX, object[index].yPos - worldY, object[index].sheetX + 4, object[index].sheetY + 3, object[index].width, 2, object[index].palette, object[index].dir, false); // Just the eyes (top row)
                        break;
                    }
                }

                if (--object[index].stunTimer == 0)
                {
                    object[index].state = 0;
                    object[index].turning = false;
            ...
            {
                if (object[index].turning == true)
                {
                    offsetX += object[index].width; // Robot turning face
                    if (--object[index].animate == 0)
                    {
                        object[index].turning = false;
                    }
                }
                ...
                if (object[index].dir == false)
                {
                    ...
                    { // Sub robot width from right edge, since we are checking left edge of robot xPos
                        ...
                        object[index].animate = 4;
                    }
                }
                else
                {
                    object[index].xPos -= object[index].speedPixels;
                    if (object[index].xPos < (object[index].xSentryLeft))
                    {
                        object[index].dir = false;
                        object[index].turning = true;
                        object[index].animate = 4;
                    }
                }
        ...
        { // Cute kitten
            ...
            if (object[index].xSentryLeft > 0)
            { // Used as a debounce when Bud whacks kitten
                --object[index].xSentryLeft;
            }
            ...
            if (object[index].state == 200)
            {
                ...
                {
                    object[index].active = false;
                }
                if (object[index].animate < 4)
                {
            ...
            else
            {
                ...
                ++object[index].subAnimate;

                if (object[index].subAnimate > 3)
                {
                    offsetX = 2;
                }

                if (object[index].subAnimate > 6)
                {
                    object[index].subAnimate = 0;
                }

                ++object[index].animate;

                if (object[index].animate > 20)
                { // 20 frames off, 20 frames help, 20 frames !!!
                    if (object[index].animate < 40)
                    {
                        drawSpriteRange(object[index].xPos - worldX, object[index].yPos - worldY - 10, 8, object[index].sheetY + 2, object[index].width, 1, 0, false, false);
                    }
                    else
                    {
                        drawSpriteRange(object[index].xPos - worldX, object[index].yPos - worldY - 10, 10, object[index].sheetY + 2, object[index].width, 1, 0, false, false);
                    }
                }

                if (object[index].animate > 60)
                {
                    object[index].animate = 0;
                }
            }
        ...
        if (object[index].type == 5 && object[index].moving == true)
        { // Greenie?

            drawSpriteSingle(object[index].xPos - worldX, object[index].yPos - worldY, 0x06, 16 + object[index].animate, 5, false, false);

            object[index].visible = true;

            if (++object[index].subAnimate > 2)
            {
                object[index].subAnimate = 0;
                if (object[index].dir == true)
                {
                    if (++object[index].animate > 6)
                    {
                        object[index].animate = 7;
                        object[index].dir = false;
                    }
                }
                else
    ...
    { // Breaking glass shards
        ...
        else
        {
            ...
            if (object[index].yPos > (119 - (object[index].height << 3)))
            {
                ...
                object[index].category = 99;
            }
        }
    }

    if (object[index].category == 100)
    { // Object has become an EXPLOSION!

        switch (object[index].animate)
        { // Animate on 2's

        case 0:
            drawSpriteRange(tempX - worldX, object[index].yPos - worldY, 6, 48 + 8, 2, 2, 4, object[index].dir, false);
            break;
        ...
        case 2:
            drawSpriteRange(tempX - worldX - 8, object[index].yPos - worldY - 8, 8, 48 + 8, 4, 4, 4, object[index].dir, false);
            break;

        case 3:
            drawSpriteRange(tempX - worldX - 8, object[index].yPos - worldY - 8, 8, 48 + 12, 4, 4, 4, object[index].dir, false);
            break;

        case 4:
            drawSpriteRange(tempX - worldX - 8, object[index].yPos - worldY - 8, 12, 48 + 12, 4, 4, 4, object[index].dir, false);
            break;
        }
    ...
    if (object[index].visible == true && object[index].category < 100)
    {
        drawSpriteRange(tempX - worldX, object[index].yPos - worldY, object[index].sheetX + offsetX, object[index].sheetY + offsetY, object[index].width, object[index].height, object[index].palette, object[index].dir, false);
    }
}
bool go_hitBox(int index, int16_t x1, int16_t y1, int16_t x2, int16_t y2)
{
    if (object[index].visible == false)
    {
        return false;
    }

    int x1Lap = x1 - object[index].xPos;
    int x2Lap = x2 - object[index].xPos;

    int pixelWidth = object[index].width << 3;
    int pixelHeight = object[index].height << 3;
    ...
    int y1Lap = y1 - object[index].yPos;
    int y2Lap = y2 - object[index].yPos;
...
bool go_hitBoxSmall(int index, int16_t x1, int16_t y1, int16_t x2, int16_t y2)
{
    ...
    if ((object[index].xPos < x1 && (object[index].xPos + object[index].width) < x1) || object[index].xPos > x2)
    { // No match on X
        return false;
    }

    if ((object[index].yPos < y1 && (object[index].yPos + object[index].height) < y1) || object[index].yPos > y2)
    { // No match on Y
        return false;
    }
...
{
    object[index].active = false;   // True = active (logic can execute, ie enemy moves)
    object[index].visible = false;  // True = onscreen false = offscreen. If false, don't run hitbox
    object[index].state = 0;        // Falling, turning, intact, destroyed, test animate etc
    object[index].whenBudTouch = 0; // 0 = nothing, 1 = knock over, 2 = collect (greenie), 3 = kill (monster)
    object[index].xPos = 0;
    object[index].yPos = 0;           // Position in world, is upper left corner of sprite
    object[index].singleTile = false; // Greenies are single cell, rest are bigger
    object[index].sheetX = 0;
    object[index].sheetY = 0;   // Upper left corner of sprite on sheet
    object[index].width = 1;    // Size in tiles
    object[index].height = 1;   // Used for hitboxes and screen calcs
    object[index].palette = 0;  // 0-7
    object[index].category = 0; ////0= gameplay (enemies and greenies), 1 = bad art 4x2, 2 = small 2x2, 3= tall 2x3, 4= appliance 3x2, 5= eraser
    object[index].type = 0;     // What kind of object this is within category
    object[index].dir = false;  // False = facing right (not flipped) Sprites should be drawn facing right as default (false)
    object[index].turning = false;
    object[index].animate = 0;
    object[index].subAnimate = 0;
    object[index].xSentryLeft = 0;
    object[index].xSentryRight = 120;
    object[index].moving = false;
    object[index].extraY = false;
    object[index].extraA = 0;
    object[index].extraB = 0;
    object[index].extraC = 0;
    object[index].extraD = 0;
